...
def update(frame):
    ...
    for i in range(MAX_AGENTS):
        ...
        # ---------------------------------------------------------
        # 내적(Dot Product)을 이용한 골고루 흡수
        # ---------------------------------------------------------
        # 개체의 유전자(RGB)와 환경의 에너지(RGB)의 유사도를 계산해 흡수
        # 섞인 색상(예: 노란색 [1,1,0])도 자신의 비율에 맞는 에너지를 먹고 생존 가능해짐
        absorption = np.dot(colors[i], world[x_pos, y_pos])
        energy[i] += absorption * 0.08  # 과다 섭취를 막기 위해 흡수율 수치 조정
